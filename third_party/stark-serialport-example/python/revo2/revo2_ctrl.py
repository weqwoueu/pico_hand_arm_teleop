...
# Motor status positions are raw normalized values [0-1000].
PRINT_ANGLE_ESTIMATE = True
# Optional estimate max angles per channel: [Thumb, ThumbAux, Index, Middle, Ring, Pinky]
EST_MAX_ANGLES_DEG = [90.0, 90.0, 81.0, 81.0, 81.0, 81.0]
...
async def all_fingers_control_examples(client, slave_id):
    ...
    positions = [0] * 6
    durations = [1000] * 6  # Expected time to reach target position (milliseconds)
    await client.set_finger_positions_and_durations(slave_id, positions, durations)
    await asyncio.sleep(2.0)  # Wait for fingers to reach target position
    await get_and_display_motor_status(client, slave_id)
    ...
    positions = [1000, 1000] + [0] * 4
    speeds = [1000] * 6  # Expected speed to reach target position
    await client.set_finger_positions_and_speeds(slave_id, positions, speeds)
    await asyncio.sleep(2.0)  # Wait for fingers to reach target position
    await get_and_display_motor_status(client, slave_id)

    positions = [1000, 0] + [0] * 4
    speeds = [1000] * 6  # Expected speed to reach target position
    await client.set_finger_positions_and_speeds(slave_id, positions, speeds)
    await asyncio.sleep(2.0)  # Wait for fingers to reach target position
    await get_and_display_motor_status(client, slave_id)
...
async def get_and_display_motor_status(client, slave_id):
    ...
    # logger.info(f"positions: {positions}")  # Position
    ...
    raw_positions = [int(p) for p in positions]
    logger.info(
        "Finger status(raw): MotorStatusData {\n"
        f"                 positions_raw(0-1000): {raw_positions},\n"
        f"                 speeds: {list(status.speeds)},\n"
        f"                 currents: {list(status.currents)},\n"
        f"                 states: {list(status.states)},\n"
        "      }"
    )
    if PRINT_ANGLE_ESTIMATE and len(raw_positions) >= 6:
        angle_estimate = [
            round(max(0.0, min(1000.0, raw_positions[i])) / 1000.0 * EST_MAX_ANGLES_DEG[i], 2)
            for i in range(6)
        ]
        logger.info(f"angle_estimate(deg, 估算): {angle_estimate}")
